...
function main()
    ...
    obj_path = "objs/lion_repaired.obj"
    tris = mesh_to_STri(load(obj_path))

	for tri in tris
		for i in 5:7
			for j in i:7
				if dot(tri[i], tri[j]) < 0
					println("!")
				end
			end
		end
    ...
    width = 128
    height = 128#Int(width * 3 / 4)
    frame_n = 720

	function moving_camera(frame_i, frame_n)
		camera_pos = V3((45, 0, 0))
		look_at = zero(V3)
		up = V3((0.0, 0.0, -1.0))
		FOV =  45.0 * pi / 180.0
	...
	#	R0 = rotation_matrix(cross(V3(1,1,0), V3(1, 1, 1)), pi/2-acos(vector_cosine(V3(1,1,1),V3(1,1,0))))
		R = rotation_matrix(V3(0, 0, 1),  pi / 2 )# * R0
        ...
        for (sky_n, s) in enumerate(skys)
            Makie.save("out/lion/$(nameof(s))/$(lpad(i, 3, "0")).png", images[sky_n])
        end
