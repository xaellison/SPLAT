# RUN FROM /
using Revise, LazyArrays, Parameters, GLMakie, CUDA, Random, NVTX
...
function main()
    ...
    # Tracing params
        ...
        R =
            rotation_matrix(ℜ³(1, 0, 0), 2 * pi * frame / 180) *
            rotation_matrix(axis, 2 * pi * 44 / 180) *
            rotation_matrix(ℜ³(0, 0, 1), pi / 4) *
            rotation_matrix(ℜ³(1, 0, 0), pi / 2) *
            rotation_matrix(ℜ³(0, 0, 1), pi / 4)
        ...
        bounding_volumes, bounding_volumes_members =
            bv_partition(tris, BV_BISCETION_COUNT; verbose = true)
