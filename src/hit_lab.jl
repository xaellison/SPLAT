...
let 
    ...
    ray_generator(x, y, λ, dv) = camera_ray(cam, 512, 512, x, y, λ, zero(ℜ³))
   ...
   # @info index_view
    ...
    hitter = DPBVHitter(CuArray, length(rays), tris, centers, members)
    ...
    #end
    #tracer.hit_idx |> Array |> join |> SHA.sha256 |> bytes2hex |> println
end