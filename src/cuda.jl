...
function next_hit(rays :: CuArray{ADRay}, n_tris :: CuArray{Tuple{I, T}}, override) where {I, T}
    dest = CuArray{Tuple{Float32, I, T}}(undef, size(rays))
    next_hit!(dest, rays, n_tris, override)
    return dest
end

function next_hit!(dest :: CuArray{Tuple{Float32, Int32, T}}, rays, n_tris:: AbstractArray{Tuple{I, T}}, override) where {I, T}
