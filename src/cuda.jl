...
function next_hit(rays :: CuArray{ADRay}, n_tris :: CuArray{Tuple{I, T}}) where {I, T}
    dest = CuArray{Tuple{Tuple{Float32, Float32}, I, T}}(undef, size(rays))
    next_hit!(dest, rays, n_tris)
    return dest
end

function next_hit!(dest :: CuArray{Tuple{Tuple{Float32, Float32}, Int32, T}}, rays, n_tris:: AbstractArray{Tuple{I, T}}) where {I, T}
